...
\IfEq{\jobname}{\detokenize{thesis}}{}{%
    \setcounterref{chapter}{chap:fundamental}
...
\begin{wrapfigure}[20]{r}{9cm}
    ...
    \vspace*{-10mm}
    \caption[Các phân lớp của Android]{Các phân lớp của Android
        \cite{GOOGL_ANDR_ARCH}}
...
\begin{wrapfigure}[3]{r}{5cm}
    ...
    \caption[Lập trình phi cấu trúc với \texttt{GOTO}]{Lập trình phi cấu trúc
        với \texttt{GOTO} \cite{NJS_SC}}
...
\begin{wrapfigure}[8]{r}{5cm}
    ...
    \caption[Sự lộn xộn của lập trình phi cấu trúc]{Sự lộn xộn của lập trình phi
        cấu trúc \cite{NJS_SC}}
...
\begin{wrapfigure}[7]{r}{5cm}
    ...
    \caption[Ba cấu trúc cơ bản: rẽ nhánh \texttt{if}, lặp \texttt{for} và gọi
        hàm]{Ba cấu trúc cơ bản: rẽ nhánh \texttt{if}, lặp \texttt{for} và gọi
        hàm \cite{NJS_SC}}
...
\begin{table}[H]
    \centering
    \caption[Hai kiểu API tương tranh hay dùng]{Hai kiểu API tương tranh hay
        dùng \cite{NJS_SC}}
...
\begin{wrapfigure}[4]{r}{5cm}
    \centering
    \vspace*{-7mm}
    \includesvg[width=\linewidth]{../images/go-schematic-and-go-to-schematic.svg}
    \vspace*{-10mm}
    \caption[Tương tranh phi cấu trúc với \texttt{goroutine} - API kiểu tương
        tranh]{Tương tranh phi cấu trúc với \texttt{goroutine} - API kiểu tương
        tranh \cite{NJS_SC}}
    \label{fig:goroutine}
\end{wrapfigure}
...
\begin{wrapfigure}[9]{l}{7cm}
    ...
    \vspace*{-10mm}
    \caption[Tương tranh cấu trúc dùng coroutine trong Kotlin]{Tương tranh cấu
        trúc dùng coroutine trong Kotlin \cite{NJS_SC}}
...
\begin{wrapfigure}[6]{r}{7cm}
    ...
    \vspace*{-10mm}
    \caption[Kiến trúc MVP]{Kiến trúc MVP \cite{WIKI_MVP}}
    \label{fig:mvp}
\end{wrapfigure}
...
\begin{wrapfigure}[7]{r}{11.5cm}
    ...
    \vspace*{-10mm}
    \caption[Kiến trúc MVVM]{Kiến trúc MVVM \cite{MUN_MVVM}}
    \label{fig:mvvm}
\end{wrapfigure}
...
\begin{wrapfigure}[13]{r}{10cm}
    ...
    \vspace*{-10mm}
    \caption[Kiến trúc Google khuyên dùng]{Kiến trúc Google khuyên dùng
        \cite{GOOGL_APP_ARCH}}
...
Các tệp truyện mà yacv đọc có định dạng CBZ, bản chất chính là tệp nén ZIP
(\textbf{C}omic \textbf{Book} \textbf{Z}IP). Do yêu cầu của các phần sau, định
dạng tệp ZIP cũng cần được trình bày ở mức cơ bản.
...
\begin{wrapfigure}[10]{r}{11cm}
    \centering
    \vspace*{-6mm}
    \includesvg[width=\linewidth,inkscapelatex=false]
        {../images/ZIP-64_Internal_Layout.svg}
    \caption[Cấu trúc tệp nén ZIP]{Cấu trúc tệp nén ZIP \cite{WIKI_ZIP}}
    \label{fig:zip-layout}
\end{wrapfigure}

Mục tiêu trên dẫn đến thiết kế \cite{PKWARE_APPNOTE} thể hiện qua
\autoref{fig:zip-layout}:
...
\begin{itemize}[resume, before = \vspace*{-\dimexpr\topsep+\partopsep\relax}]
    \item
        ...
        tin liên quan, trong đó có \emph{tên tệp gốc}.
    \item
        Ở \emph{cuối} tệp ZIP, các header được gom lại, lưu một lần nữa vào một
        cấu trúc gọi là \emph{Central Directory}. Hơn nữa, từng header còn lưu
        thêm \emph{vị trí bắt đầu} (offset), tức số byte tính từ đầu tệp ZIP đến
        tệp nén lẻ tương ứng.

        Có thể so sánh File Entry như các \emph{đề mục}, còn Central Directory
        là \emph{mục lục}. Từ đây, ta tạm dùng hai cụm từ này thay vì cụm tiếng
        Anh gốc cho dễ hiểu.
\end{itemize}
...
\begin{itemize}[resume, before = \vspace*{-\dimexpr\topsep+\partopsep\relax}]
    \item
        ...
        \begin{itemize}
            \item
                Thuận lợi cho đọc-ghi từng tệp lẻ:
                ...
                Trước đây tối ưu này rất quan trọng. Do đĩa mềm - phương tiện
                chia sẻ chủ yếu thời đó - có dung lượng nhỏ, tệp ZIP có thể phải
                cắt ra cho vừa \cite{WIKI_ZIP}. Thiết kế này cho phép chỉ sửa
                lại dữ liệu ở một số đĩa, thay vì toàn bộ.
            \item
                Cho phép ghi (nén) theo luồng xuất:

                Theo định nghĩa, mục lục tổng hợp các thông tin trong tệp nén.
                Do đó, nếu ghi ra luồng xuất, hiển nhiên nó phải ở cuối cùng.
                Nếu nó ở đầu thì lúc ghi mục lục không có thông tin gì, làm bản
                thân nó vô dụng.
        \end{itemize}
\end{itemize}
...
Cấu trúc này cũng dẫn đến hai kiểu API nén/giải nén:

\begin{enumerate}
    \item Dựa trên tệp: đọc phần mục lục trước, sau đó nhảy cóc đến vị trí
        (offset) của bất kì tệp nén lẻ nào rồi giải nén nó
    \item Dựa trên luồng: đọc lần lượt từng tệp nén lẻ theo luồng đọc, đồng thời
        phải giải nén toàn bộ tệp đó trước khi "dò" được tệp kế
\end{enumerate}

Trong các phần sau, ta sẽ thấy SAF chỉ cho phép dùng cách 2, nhưng yacv cố gắng
thiết kế để dùng cách 1.

Một điểm quan trọng nữa trong tệp ZIP, đó là gần như không kiểm soát được thứ tự
tệp nén lẻ.
...
Cũng cần nhắc lại rằng thứ tự tệp nén lẻ không được đảm bảo. Ví dụ, tệp ảnh
\texttt{1.jpg} có thể đứng sau tệp \texttt{2.jpg}.
